...
display(['-stepsize ',sprintf('%.4f,',[b_lum(1),b_lum(2:end-1)*1.03]/256),sprintf('%.4f;',b_lum(end)*1.03/256),...
...
str_intercept=['-lin_model_intercept '];
idx_reorder=[14,15,16,11,12,13,8,9,10,5,6,7,2,3,4,1];
...
for jnd_level=4:-1:1
    ...
    b_lum=b_lum(idx_reorder);
    b_c1=b_c1(idx_reorder);
    b_c2=b_c2(idx_reorder);
    
    
    str_slope=[str_slope,sprintf('%.4f,',a_lum(1:end-1)),sprintf('%.4f',a_lum(end))]
    %display(['-lin_model_intercept ',sprintf('%.115f,',[b_lum,b_c1,b_c2])])
