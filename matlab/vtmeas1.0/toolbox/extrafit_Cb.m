% For Cb component, predict the future VTs based on those that have
...
function extrafit_Cb(VTidx)
loadhead={'LL','HL','LH','HH'};
bvec=zeros(2,5,VTidx);
VTidx_top=VTidx;
for ki=1:VTidx
    if(exist([pwd '/data/' sprintf('CbVT%d.mat',ki)])~=0)
        load([pwd '/data/' sprintf('CbVT%d.mat',ki)]);
        bvec(:,:,ki)=T(:,:);
    else
        VTidx_top=ki-1;
        break;
...
if(VTidx_top>1)
    ...
    save([pwd '/data/' sprintf('Cb_non_LL_abcoeff_up_to_%d.mat',VTidx_top)],'ab','bb');
end

bvec=zeros(1,VTidx);
VTidx_top=VTidx;
for ki=1:VTidx
    if(exist([pwd '/data/' loadhead{1} sprintf('5Cbvt%d_3afc.mat',ki)])~=0);
        load([pwd '/data/' loadhead{1} sprintf('5Cbvt%d_3afc.mat',ki)]);
        bvec(ki)=t;
    else
        VTidx_top=ki-1;
        break;
...
if(VTidx_top>1)
    ...
    ab=p1(1);
    bb=p1(2);
    save([pwd '/data/' sprintf('Cb_LL_abcoeff_up_to_%d.mat',VTidx_top)],'ab','bb');
end