% For luminance component, predict the future VTs based on those that have
...
function extrafit_lum(VTidx)
avec=zeros(2,5,VTidx);
bvec=zeros(2,5,VTidx);
VTidx_top=VTidx;
for ki=1:VTidx
    if(exist([pwd '/data/' sprintf('abR_VT%d.mat',ki)])~=0)
        load([pwd '/data/' sprintf('abR_VT%d.mat',ki)]);
        avec(:,:,ki)=a(:,:);
        bvec(:,:,ki)=b(:,:);
    else
        VTidx_top=ki-1;
...
rsq=zeros(2,5);
if(VTidx_top>1)
    avec=avec(:,:,1:VTidx_top);
    bvec=bvec(:,:,1:VTidx_top);
    ba=exp(sum(log(avec),3)/VTidx_top);
    for ki=1:2
        for kj=1:5
    ...
    save([pwd '/data/' sprintf('LUM_non_LL_abcoeff_up_to_%d.mat',VTidx_top)],'aa','ba','ab','bb');
end

bvec=zeros(1,VTidx);
VTidx_top=VTidx;
for ki=1:VTidx
    if(exist([pwd '/data/' sprintf('LL5vt%d_3afc_no_1.mat',ki)])~=0);
        load([pwd '/data/' sprintf('LL5vt%d_3afc_no_1.mat',ki)]);
        avec(ki)=0;
        bvec(ki)=t;
    else
        VTidx_top=ki-1;
...
if(VTidx_top>1)
    ...
    ab=p1(1);
    bb=p1(2);
    save([pwd '/data/' sprintf('LUM_LL_abcoeff_up_to_%d.mat',VTidx_top)],'ab','bb');
end